...
def pretty_print(resp: requests.Response, title: str = ""):
    ...
    try:
        ...
        debug_print(f"Response parsed successfully", "RESPONSE")
        return data
...
def test_style_transfer_text(content_image: str = None, style_text: str = None, prompt: str = None):
...
def test_style_transfer_ref(content_image: str = None, style_image: str = None, prompt: str = None):
...
def test_classify_quick(prompt: str = None):
    ...
    debug_print(f"Prompt: {prompt}", "REQUEST")
    debug_print(f"Quick mode: True", "REQUEST")
...
def test_classify_full(prompt: str = None):
    ...
    debug_print(f"Prompt: {prompt}", "REQUEST")
    debug_print(f"Quick mode: False (full)", "REQUEST")
...
def test_sam_segment(image_path: str = None, x: int = 150, y: int = 200, output_dir: str = None):
...
def run_ai_editing_pipeline(image_path: str, user_prompt: str) -> Dict[str, Any]:
    ...
    try:
        ...
        debug_print(f"Analysis data received: {json.dumps(analysis_data, indent=2)}", "RESPONSE")
    ...
    try:
        ...
        debug_print(f"Classification response status: {classify_resp.status_code}", "RESPONSE")
        ...
        debug_print(f"Classification data: {json.dumps(classification_data, indent=2)}", "RESPONSE")
...
def execute_tool(tool, image_path, prompt, params, ai_suggestions):
    ...
    try:
        ...
        if tool == 'style-transfer-text':
            ...
            output_path = output_paths.get('composite') or output_paths.get('styled_only') or result.get('output_composite') or result.get('output_styled_only')
            ...
            output_path = (output_paths.get('output_image') or 
                          output_paths.get('composite') or
                          outputs.get('ai_graded') or
                          outputs.get('manual_graded'))
        elif tool == 'segmentation':
            output_path = result.get('output_path') or result.get('mask_path') or result.get('output') or result.get('mask')
